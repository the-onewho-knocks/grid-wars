...
const (
	TOTAL_TILES      = 1000
	CAPTURE_COOLDOWN = 1 * time.Second  // per-user cooldown between captures
...
// 	if unclaimed == 0 {
// 		go func() {
// 			// FIX 3: Recover from any panic so gameOverFired is always cleared
// 			// and the game is never permanently stuck.
// 			defer func() {
// 				if r := recover(); r != nil {
// 					log.Println("checkGameOver: panic recovered:", r)
// 					s.gameOverMu.Lock()
...
// func (s *GameService) checkGameOver(ctx context.Context) {
// 	// Re-verify all tiles are still claimed (guards against a race
// 	// where another goroutine already triggered and reset the board)
// 	var unclaimed int
// 	err := s.db.QueryRow(ctx, `SELECT COUNT(*) FROM tiles WHERE owner_id IS NULL`).Scan(&unclaimed)
// 	if err != nil || unclaimed > 0 {
// 		return
...
// 	if err != nil {
// 		log.Println("checkGameOver: leaderboard query failed:", err)
// 		// FIX 2: Always unlock gameOverFired on early return so the next
// 		// game can trigger a new game-over cycle.
// 		s.gameOverMu.Lock()
// 		s.gameOverFired = false
// 		s.gameOverMu.Unlock()
// 		return
...
// func (s *GameService) resetGame(ctx context.Context) {
// 	// FIX 2: Always clear gameOverFired whether the reset succeeds or fails,
// 	// so the game is never permanently stuck in a fired state.
// 	defer func() {
// 		s.gameOverMu.Lock()
// 		s.gameOverFired = false
// 		s.gameOverMu.Unlock()
...
// 	// FIX 1: Broadcast every tile as unclaimed so clients' grids reset
// 	// visually. Without this, clients only receive a generic "new_game"
// 	// event and never learn which tiles were cleared — the grid stays
// 	// frozen in the final claimed state.
// 	s.broadcastTileResets(ctx)
...
// // broadcastTileResets publishes a tile_update for every tile with a nil
// // owner so all connected clients reset their local grid state.
// func (s *GameService) broadcastTileResets(ctx context.Context) {
// 	rows, err := s.db.Query(ctx, `SELECT id FROM tiles`)
// 	if err != nil {
// 		log.Println("broadcastTileResets: query failed:", err)
